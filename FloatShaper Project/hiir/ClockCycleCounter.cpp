...
static inline double	hiir_ClockCycleCounter_get_time_s ()
{
...
,	_best (-1)
{
	if (! _init_flag)
	{
		...
		const double	start_time_s = hiir_ClockCycleCounter_get_time_s ();
		start ();
		...
		const double	stop_time_s = hiir_ClockCycleCounter_get_time_s ();
		stop ();
