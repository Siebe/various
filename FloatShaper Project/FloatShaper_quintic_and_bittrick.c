...
#define MAXDRIVE 3
#define TABLE_INTERP 1 //0 - no interpolation; 1 - linear; 2 - quintic;
/*****************************************************************************/
  ...
  for (i= 0; i < SampleCount; i++) {
    ...
    if (fInputSample == 0.) {
      *(pfOutput++) = 0.;
      ...
      //absolutization-trick
      lSignBit = (unsigned long) (*((int*) &fInputSample) & 0x80000000); //store the signbit seperatly
      fInputSample = (LADSPA_Data) fabs(fInputSample);
      // *((int *) &fInputSample) &= 0x7fffffff; // http://aggregate.org/MAGIC/#Absolute%20Value%20of%20a%20Float
      //omdat ik dit vergeet: 
      //- gebruik pointer naar inputsample - &InputSample
      //- typecast naar pointer van int-type - (int *) 
      //- verwijs naar waarde van pointer en bitmask deze als zijnde int -  *(x) &= y
      //- bitmask met 7fffffff behoudt alle bits behalve het signbit
//en dit werkt dus niet vanwege de optimizer strict aliasing rules, regel wordt gewoon genegeerd. Kan wel uit, gaat ten koste van optimizatie. 
//eingelijk boeit dit niet, want dit truukje werkt tog niet meer in latere versies. Echter, zoek naar no-stric-aliasing om een functie te maken
//die de regels negeert.           
      ...
      fInputIndex = fInputSample * ((LADSPA_Data)lTableSize - 1.); // input * tablehigh
      lTableIndex = (unsigned long)floor(fInputIndex);
      fWeight = fInputIndex - (LADSPA_Data)lTableIndex;
        
      //quintic interpolation
      if (TABLE_INTERP == 2) {
        *(pfOutput++) = interpolate_quintic(fWeight, pfTable[lTableIndex], pfTable[lTableIndex + 1], pfTable[lTableIndex + 2], pfTable[lTableIndex + 3]);   
      } 
      else {
        ...
        if ((lTableIndex >= (lTableSize -1)) || (fWeight < 0.001) || (TABLE_INTERP == 0)) {
          *(pfOutput++) = (pfTable[lTableIndex]);
        }
        else if (fWeight > 0.999) {
          *(pfOutput++) = (pfTable[lTableIndex + 1]);
        }
        //linear interpolation    
        else {
          *(pfOutput++) = (pfTable[lTableIndex] * (1. - fWeight)) + (pfTable[lTableIndex + 1] * fWeight);
        }
      } // end of linear or zero interpolation
      
      //reverse absolutization
      //xor should flip the sign-bit, if present. flip because part of table could be negative: neg * neg = pos, neg * pos = neg
      *((int *)pfOutput) ^= (int)lSignBit; 
