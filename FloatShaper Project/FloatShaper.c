...
#define TABLESIZE 512  //length of the tables (is only HALF a table)
#define SSBUFFERSIZE 8192 //length of the supersampling buffer 
//ssbuffersize must be at least 2x max SampleCount plus 6 (assuming most hosts use max 1024 buffer, 4096 should be more than enough)
#define MAXDRIVE 3 //amplification of the table
#define MAXDELAY 20 //in ms.. for now
...
typedef struct {
  ...
  //Delay Memory
  LADSPA_Data * m_pfDelayMem;
  //supersampling buffer
  LADSPA_Data * m_pfSSbuffer;

  unsigned long m_lTableSize; //size of the tables
  unsigned long m_lDelaySize; //size of the delay mem
  LADSPA_Data m_fDelaySize; //real size of delay mem, to calculate input
  LADSPA_Data m_fSRinMS; //sample rate in ms

  unsigned long m_lDelayPointer; //points to current write sample in delay table

  //pointer to HalfBandFilter
  HalfBandFilter * m_pHBF;    
...
instantiateFloatShaper(const LADSPA_Descriptor * Descriptor,
  ...
  //calculate and allocate memory for the delay buffer
  psFloatShaper->m_fDelaySize = (SampleRate * (LADSPA_Data) MAXDELAY * 0.001 * 2 ); //*2 because of supersampling
  lDelaySize = (unsigned long) floor(psFloatShaper->m_fDelaySize + 0.5);
  psFloatShaper->m_lDelaySize = lDelaySize;
  psFloatShaper->m_pfDelayMem = (LADSPA_Data *)calloc(lDelaySize, sizeof(LADSPA_Data));
  //store samplerate in ms for later conversion
  psFloatShaper->m_fSRinMS = (SampleRate * 0.001 * 2); //*2 because of supersampling  

  //quit if that doesn't work
  if ((psFloatShaper->m_pfDelayMem == NULL)){
    free(psFloatShaper->m_pfDelayMem);
    free(psFloatShaper->m_pfTable);
    free(psFloatShaper->m_pfLinTable);
    free(psFloatShaper->m_pfSinTable);
    free(psFloatShaper->m_pfRectTable);
  ...
  //allocate memory for the supersample buffer
  psFloatShaper->m_pfSSbuffer = (LADSPA_Data *)calloc(SSBUFFERSIZE, sizeof(LADSPA_Data));

  //quit if that doesn't work
  if ((psFloatShaper->m_pfDelayMem == NULL)){
    free(psFloatShaper->m_pfSSbuffer);
    free(psFloatShaper->m_pfDelayMem);
    free(psFloatShaper->m_pfTable);
    free(psFloatShaper->m_pfLinTable);
    free(psFloatShaper->m_pfSinTable);
  ...
  //allocate an halfbandfilter
  psFloatShaper->m_pHBF = instantiateHBF();

    //quit if that doesn't work
  if ((psFloatShaper->m_pHBF == NULL)){
    cleanupHBF(psFloatShaper->m_pHBF);
    free(psFloatShaper->m_pfSSbuffer);
    free(psFloatShaper->m_pfDelayMem);
    free(psFloatShaper->m_pfTable);
    free(psFloatShaper->m_pfLinTable);
...
connectPortToFloatShaper(LADSPA_Handle Instance,
  ...
  case FS_OVERDRIVE:
    ...
    break;
  case FS_DELAY:
    psFloatShaper->m_pfDelay = DataLocation;
    break;
  case FS_INPUT:
...
runFloatShaper(LADSPA_Handle Instance,
  ...
  LADSPA_Data * pfDelayMem;
  LADSPA_Data * pfSSbuffer;
  LADSPA_Data * pfInput;
  ...
  //declare pointer for delaymem write 'pointer'
  unsigned long * plDelayPointer;
  //other local vars
  ...
  LADSPA_Data fPoint;
  LADSPA_Data fWeight;
  LADSPA_Data fDelayReadPoint;
  unsigned long lTableSize;
  unsigned long lDelaySize;
  unsigned long lSSbufSize;
  LADSPA_Data fDelaySize;
  LADSPA_Data fSRinMS;
  ...
  //Also for the HalfBandFilter struct
  HalfBandFilter * pHBF;
  pHBF = psFloatShaper->m_pHBF;
  ...
  pfDelayMem = psFloatShaper->m_pfDelayMem;
  pfSSbuffer = psFloatShaper->m_pfSSbuffer;
  ...
  fDelay = *(psFloatShaper->m_pfDelay);
  ...
  //get delay pointer
  plDelayPointer = &(psFloatShaper->m_lDelayPointer);
  
  //some local vars
  lTableSize = psFloatShaper->m_lTableSize;
  lDelaySize = psFloatShaper->m_lDelaySize;
  fDelaySize = psFloatShaper->m_fDelaySize;
  fSRinMS = psFloatShaper->m_fSRinMS;  
...
//*****DSP stufz:
  ...
  //fill supersamplebuffer
  lSSbufSize = 2*SampleCount;
  //totally abort if this going to cost more than allocated memory, shoudn't happen
  if (lSSbufSize + 6 > SSBUFFERSIZE) {return;}
  //asume three 'old' sampes are still in the buffer, start writing at index 6
  for (i = 0 ; i < SampleCount; i++) {
    *(pfSSbuffer + i*2 + 6) = *(pfInput + i);
    }
  //use quintic interpolation to fill in the gaps, first gap at index 3, last at samplecount*2-3
  for (i = 0 ; i < SampleCount; i++) {
    *(pfSSbuffer + i*2 + 3) = interpolate_quintic(0.5, *(pfSSbuffer + i*2 + 0), *(pfSSbuffer + i*2 + 2), *(pfSSbuffer + i*2 + 4), *(pfSSbuffer + i*2 + 6));
  } 
  
  //read out the table for each sample in supersamplebuffer
  for (i= 0; i < lSSbufSize; i++) {
    
    //fInputSample = *(pfInput++);
    //use supersample buffer now!
    fInputSample = *(pfSSbuffer + i);

    //do nothing if input is zero, except for setting delaymem and incrementing the delaypointer 
    if (fInputSample == 0.) {
      pfDelayMem[(*plDelayPointer)] = 0.;
      (*plDelayPointer)++;
      if (*plDelayPointer >= lDelaySize) {*plDelayPointer = 0;}      
      //*(pfOutput++) = 0.;
      *(pfSSbuffer + i) = 0.;
    }
    else {
      ...
      //write current tablesample to the memory
      pfDelayMem[(*plDelayPointer)] = fTableSample;

      //calculate the readpointer 
      fDelayReadPoint = (LADSPA_Data) (*plDelayPointer) - (fDelay * fSRinMS);
      if (fDelayReadPoint < 0.) {fDelayReadPoint += (LADSPA_Data)lDelaySize;}

      //increase (and wrap) the writepointer
      (*plDelayPointer)++;
      if (*plDelayPointer >= lDelaySize) {*plDelayPointer = 0;}

      //linear interpolation for the delay memory reading for now; reuse of lTableIndex, fWeight and fTableSample variables!!!
      lTableIndex = (unsigned long) floor(fDelayReadPoint);
      fWeight = fDelayReadPoint - (LADSPA_Data)lTableIndex;
      //stupid workaround for making the interpolation work with the wrap
      if (lTableIndex == lDelaySize - 1) 
        {fTableSample = (pfDelayMem[lTableIndex] * (1. - fWeight)) + (pfDelayMem[0] * fWeight);}   
      else  
        {fTableSample = (pfDelayMem[lTableIndex] * (1. - fWeight)) + (pfDelayMem[lTableIndex + 1] * fWeight);}
 
      //get delayed tablesample and put it over input
      fOutputSample = fInputSample * fTableSample;

      //apply overdrive (that's afterwards yes)
      if (fOutputSample > 1.) {
        if (fOD <= .5) {
      ...
      *(pfSSbuffer + i) = fOutputSample; 
    
    }//end of non-zero sample-handing
    
  }//end of shaping-loop

  //do the halfband-filter-trick here to finish off the supersample routine
  for (i=0; i < lSSbufSize; i++) {
    *(pfSSbuffer + i) = processHBF(pHBF, *(pfSSbuffer + i));
  }

  //now write half the supersample-buffer to the output buffer and let it roll
  for (i=0; i < SampleCount; i++) {
    *(pfOutput + i) = *(pfSSbuffer + i*2);
  }
  //finally put the final five (six is always zero so forget that one) samples of the SSbuffer in front, for nex tloop
  for (i=0; i < 5; i++) {
    *(pfSSbuffer + i) = *(pfSSbuffer + i + lSSbufSize);
  }
...
cleanupFloatShaper(LADSPA_Handle Instance) {
  ...
  psFloatShaper = (FloatShaper *)Instance;
  //clean up the HalfBandFilter
  cleanupHBF(psFloatShaper->m_pHBF);
  //free the tables/buffers
  free(psFloatShaper->m_pfSSbuffer);
  free(psFloatShaper->m_pfDelayMem);
  free(psFloatShaper->m_pfTable);
  free(psFloatShaper->m_pfLinTable);
...
_init() {
  ...
  if (g_psDescriptor) {
    ...
    pcPortNames[FS_DELAY] 
      = strdup("Delay of Shaping");
    ...
    psPortRangeHints[FS_OVERDRIVE].UpperBound
      = 1;
    psPortRangeHints[FS_DELAY].HintDescriptor
      = (LADSPA_HINT_BOUNDED_BELOW 
	 | LADSPA_HINT_BOUNDED_ABOVE
	 | LADSPA_HINT_DEFAULT_MINIMUM);
    psPortRangeHints[FS_DELAY].LowerBound 
      = 0;
    psPortRangeHints[FS_DELAY].UpperBound
      = (LADSPA_Data)MAXDELAY;
