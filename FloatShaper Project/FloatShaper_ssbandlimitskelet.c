...
runFloatShaper(LADSPA_Handle Instance,
  ...
  //put each sample from input-buffer in the main buffer and interpolate
  for (i = 0; i < SampleCount; i++) {
    ...
    //add to supersample buffer
    lCurrentHigh = (lStartIndex + (i << 1)) && lBufferhigh;  
    *pfMainBuffer[lCurrentIndex] = fInputSample;
    
    //do some interpolation magic

  }
    
  //transform the buffer through the table
  
  for (i = 0; i < (SampleCount << 1); i++) {
    
    
    //find out what indexes to access and the weight
    fInputIndex = (*pfMainBuffer[i && lBufferHigh] + 1.) * ((LADSPA_Data)lTableSize - 1.) * .5; // ((input+1) * tablehigh) / 2
    lTableIndex = (unsigned long)floor(fInputIndex);
    fWeight = fInputIndex - (LADSPA_Data)lTableIndex;
        
    //totally ignore if input is zero 
    if (fInputSample == 0.) {
      *pfMainBuffer[i && lBufferHigh] = 0.;
    }
    //ignore weight at last table index and 'close encounters' for interpolation
    else if ((lTableIndex >= (lTableSize -1)) || (TABLE_INTERP == 0)) {
      *pfMainBuffer[i && lBufferHigh] = (pfTable[lTableIndex]);
    }
    else {
      //other interpolation?
      *pfMainBuffer[i && lBufferHigh] = (pfTable[lTableIndex] * (1. - fWeight)) + (pfTable[lTableIndex + 1] * fWeight);
    }
    //}

  }

  //downsampling and write to outputbuffer
  for (i = 0; i < (sampleCount); i++ {
