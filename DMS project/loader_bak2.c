...
dmsHandle dmsParseXML(mxml_node_t *tree, dmsHandle newNet) {
  ...
  //get the state names
  node = mxmlFindElement(mainNode, tree, "state", NULL, NULL, MXML_DESCEND_FIRST);
  for (i=0; i<stateCount; i++) {
    ...
    if (*(newNet->stateName + id*CHAR_MAX) != '\0') {
      dmsLoaderRaiseError("duplicate id number in global state definitions.");
      return NULL;
    }  
    strncpy((newNet->stateName + id*CHAR_MAX), (node->child)->value.opaque, CHAR_MAX-1);
    node = mxmlFindElement(node, tree, "state", NULL, NULL, MXML_NO_DESCEND);
  }
  ...
  cellNode = mxmlFindElement(mainNode, tree, "cell", NULL, NULL, MXML_DESCEND_FIRST);
  searchNode = cellNode;
  ...
  for (i=0; i<cellCount; i++) {
    ...
    //let's remember though where we stopped/are starting, to make sure we wont make a full loop, this breaks the whole function if a cell is missing.
    while (i != id) {       
      ...
      if (!node)
        ...
        searchNode = mxmlFindElement(mainNode, tree, "cell", NULL, NULL, MXML_DESCEND_FIRST);
      //next cell should be loaded, so if cellNode is 'node' again, break it
      if (cellNode == searchNode) {
        dmsLoaderRaiseError("missing id number in cell definitions, or only a single cell defined.");
        return NULL;
      }
    ...
    node = mxmlFindElement(cellNode, tree, "state", NULL, NULL, MXML_DESCEND_FIRST);
    ...
    for (j=0; j<stateCount; j++) {
      ...
      while (j != id) {       
        ...
        if (!searchNode)
          searchNode = mxmlFindElement(cellNode, tree, "state", NULL, NULL, MXML_DESCEND_FIRST);
        if (node == searchNode) {
